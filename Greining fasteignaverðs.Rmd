...
Umbreytum y út frá BoxCox og skoðum lambda gildið

```{r}
bcTest <- boxcox(lm( nuvirdi ~ ., td6))
...
Lambda er langt fyrir neðan 5 sem er góðs viti. Við athugun á endurtekna mynstri aukastafanna kemur í ljós að lambda er líklegast nákvæmlega $26/99$.


``` {r}
td6$logy <- bxcx(td6$nuvirdi, lambda, InverseQ = FALSE, type = "BoxCox")
lm.sixth = lm(logy ~ . -nuvirdi, data = td6)
...
td7 <- subset(td6, select = -c(lyfta, fjsturt))
td7$ibm22 <- td7$ibm2^2
# MJÖG significant, prófum samt að velja bara 2. veldi fyrst

lmOrthoPolyBYGGAR <- lm(td6$logy ~ poly(td6$byggar, 3))
